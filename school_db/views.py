...
def problem_three(request):
    ...
    instructors = Instructor.objects.filter(pk = 2)
    course = Course.objects.all()
    for instructor in instructors:
      print(f'Instructor name: {instructor.first_name}  {instructor. last_name }')
      print(f'Courses: Science, History')
...
def problem_five(request):
    student = Student.objects.create(id = 11, first_name = 'Niguel', last_name = 'Price', year = 2022, gpa = 4.0)
    student.save()
...
def problem_six(request):
    ...
    student = Student.objects.filter(id = 11).update(gpa = 4.3)
    student = Student.objects.filter(id = 11).get()
